...
class CombatTimeline {
    ...
    updateCombatants(combat) {
        ...
        // Utiliser directement l'ordre du combat tel que défini par le système de jeu
        let combatants;
        if (game.combat) {
            // Log de debug pour voir la structure complète
            console.log("Sci-Fi Battle Timeline | Structure du combat:", {
                turns: game.combat.turns,
                combatants: game.combat.combatants,
                current: game.combat.current,
                started: game.combat.started,
                settings: game.combat.settings
            });
            
            // Utiliser l'ordre exact tel que défini par le système de jeu
            combatants = game.combat.turns;
            
            // Log pour debug - Ordre dans Foundry
            console.log("Sci-Fi Battle Timeline | Ordre dans Foundry:");
            game.combat.turns.forEach(c => {
                console.log(`- ${c.name} (initiative: ${c.initiative})`);
            });
        } else {
            // Si pas de combat actif, utiliser les combattants dans leur ordre d'origine
        ...
        // Log pour debug - Ordre dans notre module
        console.log("Sci-Fi Battle Timeline | Ordre dans notre module:");
        (combatants || []).forEach(c => {
            console.log(`- ${c.name} (initiative: ${c.initiative})`);
        });

        // Créer les éléments pour chaque combatant en respectant l'ordre du système
        for (let combatant of (combatants || [])) {
            if (!combatant || !combatant.token) continue;
            ...
            // Ajouter à la fin du conteneur au lieu du début pour préserver l'ordre
